...
def prompt_microsurvey():
    """Send a notification prompting the user to fill a micro survey."""
    notify("How are you feeling?", "Tıklayarak duygu & bağlam anketini doldurun.")


def start_microsurvey_prompts():
    interval = td2h  # every 2 hours
    ...
    def prompt_loop():
        while True:
            ...
            try:
                ...
                logger.warning(f"Micro survey prompt failed: {e}")

    t = threading.Thread(target=prompt_loop, daemon=True)
    t.start()

# Start prompts when notifier starts
start_microsurvey_prompts()
